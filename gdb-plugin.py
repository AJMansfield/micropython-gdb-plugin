...
class FeedStdinCmd(gdb.Command):
    def __init__(self):
        super(FeedStdinCmd, self).__init__(
            "feed_stdin", gdb.COMMAND_USER
        )
    ...
    def invoke(self, args, from_tty):
        ...
        stdin = gdb.lookup_symbol("stdin_ringbuf")[0].value()
        size = int(stdin["size"])

        if len(args) > size:
            raise Error("full")

        gdb.selected_inferior ().write_memory(int(stdin["buf"]), args);
        gdb.selected_inferior ().write_memory(int(stdin["iput"].address), struct.pack("<H", len(args)));
        gdb.selected_inferior ().write_memory(int(stdin["iget"].address), b"\x00\x00");
        if int(debugging.value()) & 1 != 0:
            gdb.execute("continue")
            gdb.execute("poll_stdout")

FeedStdinCmd()

class PollStdoutCmd(gdb.Command):
    def __init__(self):
        super(PollStdoutCmd, self).__init__(
            "poll_stdout", gdb.COMMAND_USER
        )
    ...
    def invoke(self, args, from_tty):
        stdout = gdb.lookup_symbol("stdout_ringbuf")[0].value()
        num = int(stdout["iput"])

        data = gdb.selected_inferior ().read_memory(int(stdout["buf"]), num);
        gdb.write(data.tobytes().decode("ASCII"))
        print()
        gdb.selected_inferior ().write_memory(int(stdout["iput"].address), b"\x00\x00");
        

PollStdoutCmd()

gdb.execute("set $iobank0_hw = ((iobank0_hw_t *) 0x40014000)")
gdb.execute("set $sio_hw = ((sio_hw_t*) 0xd0000000)")
gdb.execute("set $cyw_state = ((cyw43_int_t *) &cyw43_state)")
...
tracefile = None
def writepkg(tracefile, data):
    import time
    tm = time.time_ns()//1000;
    tracefile.write(struct.pack("<IIII", tm // 1000000, tm % 1000000, len(data), len(data)))
    tracefile.write(data)
    tracefile.flush()

@handler("cyw43_eth_output_hook")
def eth_output():
    if tracefile is None:
        return
    pbuf = gdb.selected_frame().read_var("p").dereference()
    buflen = pbuf["len"]
    buf = pbuf["payload"]
    writepkg(tracefile, gdb.selected_inferior().read_memory(int(buf), int(buflen)).tobytes())

@handler("cyw43_eth_input_hook")
def eth_input():
    if tracefile is None:
        return
    buflen = gdb.parse_and_eval("len")
    buf = gdb.selected_frame().read_var("buf")
    writepkg(tracefile, gdb.selected_inferior().read_memory(int(buf), int(buflen)).tobytes())
...
class Capture(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        global tracefile
        # tail -n+0 -f /tmp/picotrace.pcap | wireshark -k -i -
        # cyw43_cb_process_ethernet
        tracefile = open("/tmp/picotrace.pcap", "wb")
        tracefile.write(struct.pack("<IHHiIII", 0xa1b2c3d4, 2, 4, 0, 0, 65535, 1))
        gdb.execute("set debugging=debugging|0x0c")

@handler("cyw43_bt_input_hook")
def bluetooth_input():
    if bluetracefile is None:
        return
    buflen = gdb.parse_and_eval("*len")
    buf = gdb.selected_frame().read_var("buf")
    if buflen == 0:
    ...
    buf += 3
    buflen -= 3
    writepkg(bluetracefile, b"\x00\x00\x00\x01" + gdb.selected_inferior().read_memory(int(buf), int(buflen)).tobytes())

@handler("cyw43_bt_output_hook")
def bluetooth_input():
    if bluetracefile is None:
        return
    buflen = gdb.parse_and_eval("len")
    ...
    buf += 3
    buflen -= 3
    writepkg(bluetracefile, b"\x00\x00\x00\x00" + gdb.selected_inferior().read_memory(int(buf), int(buflen)).tobytes())


class BlueCapture(gdb.Command):
    def __init__(self):
        super(BlueCapture, self).__init__(
            "bpcap", gdb.COMMAND_USER
        )
    ...
    def invoke(self, args, from_tty):
        global bluetracefile
        # tail -n+0 -f /tmp/picoblue.pcap | wireshark -k -i -
        # cyw43_cb_process_ethernet
        bluetracefile = open("/tmp/picoblue.pcap", "wb")
        bluetracefile.write(struct.pack("<IHHiIII", 0xa1b2c3d4, 2, 4, 0, 0, 65535, 0xc9))
        gdb.execute("set debugging=debugging|0x30")

Capture()
BlueCapture()
# import tools; tools.updateAll("v1")
# import base; app = base.MQTTApp(); app.keepalive = 5*60; app.boot(); app.run_energysave()
